...
def run_slam(src_dir, log_dir, idx, split):
    ...
    plt.xlim([0, 800])   #limits of the plot
    plt.ylim([0, 800])

    plt.ion()    #animate the plot

    environment = plt.imshow((1 - slam.map.cells) * 255, cmap='gray')

    x_odom = slam.odom[0, :]
    y_odom = slam.odom[1, :]

    odometry,_ = plt.plot(y_odom, 800 - x_odom, '*', markersize=1, color='blue')

    x_traj = slam.robot_trajectory[0, :]
    y_traj = slam.robot_trajectory[1, :]

    pose, = plt.plot(y_traj, 800 - x_traj, '*', markersize=1, color='green')

    for t in tqdm.tqdm(range(1, len(slam.lidar))):
        slam.dynamics_step(t)
        slam.observation_step(t)            #calling functions
        image = deepcopy(slam.map.cells)
        image = (1 - image) * 255
        image[slam.unkown_cells] = 255 // 2

        image = np.flip(image, 0)
        environment.set_data(image)
        x_odom = slam.odom[0, :]           #plotting odometry
        y_odom = slam.odom[1, :]

        odometry.set_data(y_odom, 800 - x_odom)
        x_trajectory = slam.robot_trajectory[0, :]    #plotting the slam robot trajectory
        y_trajectory = slam.robot_trajectory[1, :]    #plotting slam trajectory for robot

        pose.set_data(y_trajectory, 800 - x_trajectory)
        plt.draw()
        plt.pause(0.01)
